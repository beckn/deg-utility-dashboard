...
import type { AssetMarker, MeterWithTransformer } from "../lib/types" // Changed FeederData to AssetMarker
...
export function DashboardMap({ assets, filter, onSelectMeter }: DashboardMapProps) { // Changed feeders to assets
  const [isClient, setIsClient] = useState(false)

  // Import the map component dynamically to avoid SSR issues with Leaflet
  const DynamicUtilityMap = dynamic(() => import("./utility-map").then((mod) => mod.UtilityMap), {
    ssr: false,
    loading: () => (
  ...
  return <DynamicUtilityMap assets={filteredAssets} onSelectMeter={onSelectMeter} />
}
