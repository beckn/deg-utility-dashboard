    ...
    {sidebarOpen && (
      ...
      >
        ...
        <nav className="flex flex-col gap-2 mt-4 px-4">
          <button
            className={`flex items-center gap-3 px-3 py-2 rounded-md hover:bg-muted transition-colors text-primary font-medium w-full justify-between cursor-pointer ${
              activePanel === "profile" ? "" : ""
            }`}
            onClick={() => {
      ...
      {openModal && (
        <aside
          className="fixed top-0 left-64 h-full min-w-[400px] max-w-full bg-card border-r border-border shadow-lg transform transition-transform duration-200 z-9999"
          style={{ transitionProperty: "transform" }}
          onClick={(e) => e.stopPropagation()}
        >
          {/* <div className="flex items-center justify-between px-6 h-16 border-b border-border">
