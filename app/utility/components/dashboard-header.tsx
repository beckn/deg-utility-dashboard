...
export function DashboardHeader({ onSelectPanel, activePanel }: DashboardHeaderProps) {
  ...
  const [openModal, setOpenModal] = useState<null | 'profile' | 'audit' | 'controls'>(null);
  ...
  return (
    <>
      ...
      {sidebarOpen && (
        <div
          className="fixed inset-0 z-40 bg-[#1E293B] bg-opacity-40 transition-opacity"
      ...
      {openModal && (
        <aside
          className="fixed top-0 left-0 z-50 h-full w-[400px] max-w-full bg-card border-r border-border shadow-lg transform transition-transform duration-200"
          style={{ transitionProperty: 'transform' }}
          onClick={e => e.stopPropagation()}
        >
          <div className="flex items-center justify-between px-6 h-16 border-b border-border">
            <span className="text-lg font-semibold text-primary capitalize">{openModal}</span>
